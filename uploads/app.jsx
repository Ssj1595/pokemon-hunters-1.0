...
function Badge({ kind, value }) {
  let emoji = "";
  if (kind === "habitat") emoji = HABITAT_META[value]?.emoji;
  else if (kind === "behavior") emoji = BEHAVIOR_META[value]?.emoji;
  else if (kind === "region") emoji = REGION_META[value]?.emoji;
  return (
...
function App() {
  ...
  const [uploads, addPhoto, removePhoto, clearAllPhotos] = useUploads();
  ...
  const visible = useMemo(() => {
    ...
    } else if (sort === "newest") arr.sort((a, b) => parseDate(b.date) - parseDate(a.date) || a.num - b.num);
    else if (sort === "oldest") arr.sort((a, b) => parseDate(a.date) - parseDate(b.date) || a.num - b.num);
    else if (sort === "alpha") arr.sort((a, b) => a.name.localeCompare(b.name));
    else if (sort === "rarity") arr.sort((a, b) => RARITY_ORDER.indexOf(a.rarity) - RARITY_ORDER.indexOf(b.rarity) || a.num - b.num);
  ...
  const onSubmitBird = (bird) => {
    if (editTarget) {
      ...
      if (editTarget.custom) {
        ...
        setEdit(num, patch);
        // also update uploads if photos changed
        if (patch.photos) {
          try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ ...uploads, [num]: patch.photos })); } catch {}
          window.location.reload(); // simplest: reload to refresh uploads state
        }
      }
      setEditTarget(null);
    } else {
    ...
    <>
      ...
      {window.AddBirdModal && (
        <window.AddBirdModal
          ...
          onDelete={(b) => {
            if (b.custom) removeCustomBird(b.num);
            else hideBuiltin(b.num);
            setMobileSheet(null);
          }}
          nextNum={nextNum}
          editBird={editTarget}
